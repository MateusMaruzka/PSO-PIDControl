...
for r, row in enumerate(axes):
    for c, ax in enumerate(row):
            ...
            ax.plot(x1, x2, color = 'r', label = "Círculo\nUnitário")    
            ...
            # ax.set_xticks(np.arange(-1, 1.5, step = 0.5), minor=False)
            # ax.set_yticks(np.arange(-1, 1.5, step = 0.5), minor=False)
